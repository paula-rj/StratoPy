...
class CloudClass:
    def __init__(self, hdf_path):
        self.path=hdf_path
        self.file_name = os.path.split(self.hdf_path)[-1]
    ...
    def read(self):
        ...
        #Read v data
        hdf_file = HDF(self.hdf_path, HC.READ)
        vs = hdf_file.vstart()
        ...
        def convert_coordinates(self, layers_df, projection="+proj=geos +h=35786023.0 +lon_0=-75.0"):
            ...
            """
            geo_df = gpd.GeoDataFrame(layers_df, geometry=gpd.points_from_xy(layers_data.Longitude, 
        ...
        def plot_statistics(self):
            #saca algunas estadisticas, lo que propuso José
            pass
        ...
        def plot_projection(self, geodf, layer):
            ...
            sns.scatterplot(x='Longitude', y='Latitude', data=geodf, hue=layer_str,
                            palette='bright',s=2, transform=ccrs.PlateCarree())
            axis.set_title(f"year {self.year}; day {self.julian_day}; hour {self.hour_utc}; {self.light}" )
            plt.show()
